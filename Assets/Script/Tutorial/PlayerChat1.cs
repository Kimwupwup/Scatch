...
public class PlayerChat1 : MonoBehaviour
{
    ...
    void Start() {
        ...
        dialog.Add("앞에 계단이 한개 있어요!");
        dialog.Add("계단을 오르려면 점프해서 앞으로 가야겠죠?");
        dialog.Add("[JUMP] 블럭을 사용하면 한칸 위로 점프할 수 있어요!");
        dialog.Add("좌측 상단에 있는 메뉴버튼을 눌러 위에 보이는 코드와 같이 만들어주세요!");
        dialog.Add("블럭 아래에 다른 블럭을 올리면 자동으로 연결되요!");
